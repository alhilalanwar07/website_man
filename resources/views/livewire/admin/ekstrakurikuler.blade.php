<div x-data="adminConfirmModal()">
    <div class="sr-only" role="status" aria-live="polite" aria-atomic="true">
        <span wire:loading wire:target="tab">Sedang memuat tab Ekstrakurikuler.</span>
        <span wire:loading wire:target="search,previousPage,nextPage,gotoPage">Sedang memuat daftar ekstrakurikuler.</span>
        <span wire:loading wire:target="createEkskul,editEkskul,saveEkskul,deleteEkskul">Sedang memproses data ekstrakurikuler.</span>
        <span wire:loading wire:target="createKategori,editKategori,saveKategori,deleteKategori">Sedang memproses data kategori ekstrakurikuler.</span>
    </div>
    ...
    <div wire:loading.flex wire:target="tab,search,previousPage,nextPage,gotoPage" class="mb-4 items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2 text-sm font-medium text-blue-700 dark:border-blue-900/50 dark:bg-blue-900/20 dark:text-blue-300">
        ...
        <span>Menyegarkan data Ekstrakurikuler...</span>
    </div>

    {{-- Tabs --}}
    <div class="flex gap-2 mb-6">
        <button wire:click="$set('tab', 'ekskul')" wire:loading.attr="disabled" wire:target="tab" class="px-4 py-2 text-sm font-semibold rounded-lg transition disabled:opacity-60 {{ $tab === 'ekskul' ? 'bg-blue-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700' }}">Ekstrakurikuler</button>
    ...
    <div class="flex items-center justify-between mb-4">
        <input wire:model.live.debounce.300ms="search" wire:loading.attr="disabled" wire:target="search" type="text" placeholder="Cari ekstrakurikuler..." class="px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white w-64 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
        <button wire:click="createEkskul" wire:loading.attr="disabled" wire:target="createEkskul" class="px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition disabled:opacity-60">
            <span wire:loading.remove wire:target="createEkskul">+ Tambah Ekstrakurikuler</span>
            <span wire:loading wire:target="createEkskul">Membuka...</span>
        </button>
    </div>
    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div wire:loading.block wire:target="tab,search,previousPage,nextPage,gotoPage" class="p-4">
            <x-admin.skeleton.table :columns="6" :rows="8" :key-prefix="'ekskul-table'" :show-actions="true" />
        </div>
        ...
        <div wire:loading.remove wire:target="tab,search,previousPage,nextPage,gotoPage">
            <table class="w-full text-sm">
                <thead class="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                    <tr>
                        <th class="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-300">Nama Ekstrakurikuler</th>
                        ...
                        <th class="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-300">Keterangan</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @forelse($ekskulList as $item)
                    <tr wire:key="ekskul-row-{{ $item->id }}" class="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                        ...
                        <td class="px-4 py-3 text-slate-500 dark:text-slate-400">{{ $item->deskripsi ? \Illuminate\Support\Str::limit($item->deskripsi, 50) : '-' }}</td>
                        ...
                        <td class="px-4 py-3 text-right space-x-1">
                            <button wire:click="editEkskul({{ $item->id }})" wire:loading.attr="disabled" wire:target="editEkskul({{ $item->id }})" class="px-3 py-1 text-xs font-medium bg-yellow-50 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-300 rounded-lg hover:bg-yellow-100 transition disabled:opacity-60">
                                <span wire:loading.remove wire:target="editEkskul({{ $item->id }})">Edit</span>
                                <span wire:loading wire:target="editEkskul({{ $item->id }})">Memuat...</span>
                            </button>
                            <button @click="openConfirm('deleteEkskul', {{ $item->id }}, 'Hapus ekstrakurikuler ini?', 'Data akan dihapus permanen.', 'Ya, Hapus', 'danger')" class="px-3 py-1 text-xs font-medium bg-red-50 dark:bg-red-900 text-red-700 dark:text-red-300 rounded-lg hover:bg-red-100 transition">Hapus</button>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="6" class="px-4 py-8 text-center text-slate-400">Belum ada data ekstrakurikuler.</td></tr>
                    @endforelse
                </tbody>
            </table>
            <div class="px-4 py-3 border-t border-slate-200 dark:border-slate-700">{{ $ekskulList->links() }}</div>
        </div>
    </div>
    @else
    {{-- Kategori Tab --}}
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div wire:loading.block wire:target="tab,createKategori,editKategori,saveKategori,deleteKategori" class="p-4">
            <x-admin.skeleton.table :columns="3" :rows="6" :key-prefix="'ekskul-kategori-table'" :show-actions="true" />
        </div>
        ...
        <div wire:loading.remove wire:target="tab,createKategori,editKategori,saveKategori,deleteKategori">
            <table class="w-full text-sm">
                <thead class="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                    <tr>
                        <th class="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-300">Kategori</th>
                        <th class="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-300">Jumlah Ekskul</th>
                        <th class="px-4 py-3 text-right font-semibold text-slate-600 dark:text-slate-300">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @forelse($kategoriList as $kat)
                    <tr wire:key="ekskul-kategori-row-{{ $kat->id }}" class="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                        <td class="px-4 py-3 font-medium text-slate-800 dark:text-white">{{ $kat->nama_kategori }}</td>
                        <td class="px-4 py-3 text-slate-500 dark:text-slate-400">{{ $kat->ekskul_count }}</td>
                        <td class="px-4 py-3 text-right space-x-1">
                            ...
                            <button @click="openConfirm('deleteKategori', {{ $kat->id }}, 'Hapus kategori ini?', 'Kategori ekstrakurikuler akan dihapus permanen.', 'Ya, Hapus', 'warning')" class="px-3 py-1 text-xs font-medium bg-red-50 dark:bg-red-900 text-red-700 dark:text-red-300 rounded-lg hover:bg-red-100 transition">Hapus</button>
                        </td>
                    </tr>
                    @empty
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <div class="relative bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-lg mx-4 p-6 border border-slate-200 dark:border-slate-800">
            <div wire:loading.flex wire:target="saveEkskul" class="absolute inset-0 z-10 items-center justify-center rounded-xl bg-white/75 dark:bg-slate-900/75">
                <div class="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100">Menyimpan data ekstrakurikuler...</div>
            </div>

            <h3 class="text-lg font-bold text-slate-800 dark:text-white mb-4">{{ $editId ? 'Edit' : 'Tambah' }} Ekstrakurikuler</h3>
            <form wire:submit="saveEkskul" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Nama Ekstrakurikuler *</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Peminatan *</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Keterangan Biaya</label>
                        <input wire:model="harga_estimasi" type="number" step="0.01" placeholder="0 = gratis" class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
                    </div>
                    <div>
                        ...
                        <select wire:model="status_ketersediaan" class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
                            ...
                            <option value="pre-order">Pendaftaran Dibuka</option>
                            <option value="arsip">Tidak Aktif</option>
                        </select>
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Foto / Poster</label>
                    ...
                    @if($existing_gambar)
                        <img src="{{ Storage::url($existing_gambar) }}" class="mt-2 h-16 rounded-lg object-cover" alt="Foto ekskul">
                    @endif
                </div>
                <div class="flex justify-end gap-2 pt-2">
                    <button type="button" wire:click="$set('showModal', false)" wire:loading.attr="disabled" wire:target="saveEkskul" class="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition disabled:opacity-60">Batal</button>
                    <button type="submit" wire:loading.attr="disabled" wire:target="saveEkskul" class="px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition disabled:opacity-60">
                        <span wire:loading.remove wire:target="saveEkskul">Simpan</span>
                        <span wire:loading wire:target="saveEkskul">Menyimpan...</span>
                    </button>
                </div>
            </form>
        </div>
