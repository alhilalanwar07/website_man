<div wire:poll.10s class="h-full flex flex-col min-h-[calc(100vh-8rem)]">
    <div role="status" aria-live="polite" aria-atomic="true" class="sr-only">
        <span wire:loading wire:target="search,statusFilter,periodFilter,trackFilter,programFilter,selectSiswa,closeDetail">Memuat data pendaftar.</span>
        <span wire:loading wire:target="verifySelected,rejectSelected,verifySingle,rejectSingle,saveSiswaManual,sendSecureDocumentEmailToSelected">Memproses perubahan data pendaftar.</span>
    </div>

    <div wire:loading.flex wire:target="search,statusFilter,periodFilter,trackFilter,programFilter,selectSiswa,closeDetail,verifySelected,rejectSelected,saveSiswaManual,sendSecureDocumentEmailToSelected" class="mb-4 items-center gap-2 rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm font-semibold text-blue-700 dark:border-blue-900 dark:bg-blue-900/30 dark:text-blue-300">
    ...
    <div class="relative flex-1 overflow-hidden grid grid-cols-1 {{ $selectedSiswaId ? 'lg:grid-cols-12' : 'lg:grid-cols-1' }} gap-6 transition-all duration-300">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 flex flex-col h-full {{ $selectedSiswaId ? 'lg:col-span-4' : 'lg:col-span-1 border-b' }}">
                ...
                <div class="flex flex-col gap-3">
                    <div class="grid w-full min-w-0 grid-cols-1 gap-2 {{ $selectedSiswaId ? 'sm:grid-cols-1' : 'sm:grid-cols-2 2xl:grid-cols-4' }}">
                        ...
                        <select wire:model.live="periodFilter" wire:loading.attr="disabled" wire:target="search,statusFilter,periodFilter,trackFilter,programFilter" class="h-11 w-full min-w-0 rounded-xl border-slate-300 pr-9 text-sm focus:border-blue-500 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-950 dark:text-white disabled:cursor-not-allowed disabled:opacity-60">
                            ...
                            @foreach($periodFilterOptions as $periodOption)
                                @php
                                    $gelombangLabel = $periodOption->gelombang_label ?: ('Gelombang ' . ($periodOption->gelombang_ke ?: '-'));
                                @endphp
                                <option value="{{ $periodOption->id }}">{{ $periodOption->tahun_ajaran }} - {{ $gelombangLabel }}{{ $periodOption->is_active ? ' (Aktif)' : '' }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            ...
                            <button wire:click="rejectSelected" wire:loading.attr="disabled" wire:target="rejectSelected" class="px-3 py-1.5 bg-rose-100 text-rose-700 font-bold rounded-lg text-xs hover:bg-rose-200 disabled:cursor-not-allowed disabled:opacity-60">
                                <span wire:loading.remove wire:target="rejectSelected">Tolak Massal</span>
                                <span wire:loading wire:target="rejectSelected">Memproses...</span>
                            </button>
                        </div>
                        @endif
                    </div>
            ...
            <div class="flex-1 overflow-y-auto">
                <div wire:loading.block wire:target="search,statusFilter,periodFilter,trackFilter,programFilter,selectAll,page" class="p-4">
                    <x-admin.skeleton.table :columns="6" :rows="8" />
                </div>

                <div wire:loading.remove wire:target="search,statusFilter,periodFilter,trackFilter,programFilter,selectAll,page">
                    @if(!$selectedSiswaId)
                        ...
                        <table class="w-full text-left text-sm text-slate-500 dark:text-slate-400">
                            <thead class="bg-white sticky top-0 text-xs uppercase text-slate-400 font-bold dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 z-10">
                                <tr>
                                    <th class="p-4 w-10"><input wire:model.live="selectAll" wire:loading.attr="disabled" wire:target="search,statusFilter,periodFilter,trackFilter,programFilter,selectAll,page" type="checkbox" class="rounded border-slate-300 text-blue-600 focus:ring-blue-600 disabled:cursor-not-allowed disabled:opacity-60"></th>
